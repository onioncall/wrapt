...
func Wrap(text string, terminalWidth int) string {
	...
	for range 10000 {
		// Check for actual line breaks in the upcoming section
		nextNewline := strings.Index(text[pos:], "\n")

		// If there's a newline before our next wrap point, reset from there
		if nextNewline != -1 && nextNewline < terminalWidth {
			pos += nextNewline + 1 // Resetting curson after newline
			continue
		}
